...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT-BGP-MODELS_xxx
namespace = Collection("nautobot_bgp_models")
namespace.configure(
    {
        "nautobot_bgp_models": {
            "nautobot_ver": "1.5.10",
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.nautobot_bgp_models.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_bgp_models.python_ver}...")
...
def destroy(context):
    ...
    context.run("docker network prune --force")
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml nautobot_bgp_models'
    run_command(context, command)
...
def unittest(context, keepdb=False, label="nautobot_bgp_models", failfast=False, buffer=True):
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'nautobot_bgp_models/*' --omit *migrations*"
